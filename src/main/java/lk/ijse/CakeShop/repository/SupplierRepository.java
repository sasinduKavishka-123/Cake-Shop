...
public interface SupplierRepository extends JpaRepository<Supplier, Long> {
    ...
    @Query(value = "SELECT COUNT(s.supplierId) FROM Supplier s WHERE s.supplierStatus='ACTIVE'")
    ...
    @Query(value = "SELECT new lk.ijse.CakeShop.dto.SupplierDTO(" +
            ...
            "FROM Supplier s WHERE (?1 IS NULL OR s.companyName LIKE %?1%) AND " +
            "(?2 IS NULL OR s.supplierName LIKE %?2%) AND" +
            "(?3 IS NULL OR str(s.supplierStatus) LIKE CONCAT('%', ?3, '%'))")
